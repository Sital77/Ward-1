// ══════════════════════════════════════════════════════
//  script-apangata-sifarish.js
//  अपाङ्गता परिचयपत्र सिफारिस — Firebase Firestore Logic
// ══════════════════════════════════════════════════════
...
// Real-time listener for apangataRecords
db.collection("apangataRecords").onSnapshot((snapshot) => {
    globalDatabase = [];
...
function updateDoc() {
    ...
    const problem = document.getElementById('inProblem').value      || '';
    ...
    const recOffice = document.getElementById('inReceiverOffice').value || '';
    const recLoc = document.getElementById('inReceiverLocation').value || '';
    document.getElementById('lblReceiverName').innerText = recName;
    document.getElementById('lblReceiverOffice').innerText = recOffice;
    document.getElementById('lblReceiverLocation').innerText = recLoc ? (recLoc + ' ।') : '';

    // Name and Problem
    document.getElementById('lblName').innerText = name || '....................';
    document.getElementById('lblProblem').innerText = problem || '....................';
...
async function printAndSaveSystem() {
    ...
    const obj = {
        ...
        receiverName:    document.getElementById('inReceiverName').value.trim()  || '',
        receiverOffice:  document.getElementById('inReceiverOffice').value.trim()|| '',
        receiverLocation:document.getElementById('inReceiverLocation').value.trim()|| '',
        name:            name,
        ...
        subject:         "अपाङ्गता परिचयपत्र सिफारिस",
    ...
    try {
        if (recordId !== "") {
            await db.collection("apangataRecords").doc(recordId).update(obj);
            document.getElementById('editRecordIndex').value = "";
            document.getElementById('formMainTitle').innerText = "♿ अपाङ्गता परिचयपत्र सिफारिस";
        } else {
            await db.collection("apangataRecords").add(obj);
        }
        window.print();
    } catch (e) {
        console.error(e);
    ...
    globalDatabase.forEach((rec) => {
        ...
        tbody.insertAdjacentHTML('beforeend', `
            <tr>
                ...
                <td><b>${rec.name || '-'}</b></td>
                <td><span style="color:#2b6cb0; font-weight:bold;">${rec.subject || 'अपाङ्गता परिचयपत्र सिफारिस'}</span></td>
                <td>${rec.miti || '-'}</td>
                <td>
...
function editFromDB(id) {
    ...
    document.getElementById('inReceiverName').value = rec.receiverName || '';
    document.getElementById('inReceiverOffice').value = rec.receiverOffice || '';
    document.getElementById('inReceiverLocation').value = rec.receiverLocation || '';
    document.getElementById('inName').value         = rec.name || '';
    document.getElementById('inProblem').value      = rec.problem || '';
...
async function deleteFromDB(id) {
    if (confirm("के तपाईं यो रेकर्ड क्लाउड डाटाबेसबाट स्थायी रूपमा हटाउन चाहनुहुन्छ?")) {
        try {
            await db.collection("apangataRecords").doc(id).delete();
        } catch (e) {
            console.error(e);
